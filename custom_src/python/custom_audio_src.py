...
try:
    ...
    Gst.error('custom_audio_src requires numpy')
    raise
...
class AudioTestSrc(GstBase.BaseSrc):
    __gstmetadata__ = ('custom_audio_src', 'Src',
    ...
    def do_set_caps(self, caps):
        # We technically could have done this directly in init,
        # as we already know what the result of the negotiation will be,
        # however if in the future we decided to make things more dynamic,
        # for example by supporting multiple sample formats,
        # the audio info would need to be initialized at the end
        # of the negotiation process, as we do here
    ...
    def do_start(self):
        # This means we only need to initialize here those variables that
        # will not need to be reinitialized when the element switches states.
        # We only declare and (re)initialize other variables in the do_start
    ...
    def do_gst_base_src_query(self, query):
        ...
        # this function is query in cpp.
        if query.type == Gst.QueryType.LATENCY:
            latency = Gst.util_uint64_scale_int(
                self.generate_samples_per_buffer,
    ...
    def do_create(self, offset, length):
        ...
        if not self.mute:
            r = np.repeat(
                ...
                self.info.channels)
            data = ((np.sin(2 * np.pi * r * self.freq
                            / self.info.rate) * self.volume)
                    .astype(np.float32))
        else:
            data = [0] * bytes_
        ...
        # The interesting part here is that GstBaseSrc expects us
        # to return a tuple made of (Gst.FlowReturn.OK, output_buffer)
        # if everything went well, otherwise typically
        # (Gst.FlowReturn.ERROR, None) if there was an issue
        # generating the data.
        return (Gst.FlowReturn.OK, buf)


__gstelementfactory__ = ("custom_audio_src", Gst.Rank.NONE, AudioTestSrc)
